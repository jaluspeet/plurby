...
(function (global, factory) {
  ...
  */
  ...
  /**
   * --------------------------------------------------------------------------
   * Bootstrap util/Plurby-factory.js
   * Licensed under MIT (https://github.com/twbs/bootstrap/blob/main/LICENSE)
   * --------------------------------------------------------------------------
   */
  ...
  class PlurbyFactory extends Config {
    constructor(config) {
      super();
      this._config = this._getConfig(config);
    }
    ...
    toHtml() {
      const PlurbyWrapper = document.createElement('div');
      PlurbyWrapper.innerHTML = this._maybeSanitize(this._config.Plurby);
      for (const [selector, text] of Object.entries(this._config.content)) {
        this._setContent(PlurbyWrapper, text, selector);
      }
      const Plurby = PlurbyWrapper.children[0];
      const extraClass = this._resolvePossibleFunction(this._config.extraClass);
      if (extraClass) {
        Plurby.classList.add(...extraClass.split(' '));
      }
      return Plurby;
    }
    ...
    _setContent(Plurby, content, selector) {
      const PlurbyElement = SelectorEngine.findOne(selector, Plurby);
      if (!PlurbyElement) {
        return;
      }
      content = this._resolvePossibleFunction(content);
      if (!content) {
        PlurbyElement.remove();
        return;
      }
      if (isElement$1(content)) {
        this._putElementInPlurby(getElement(content), PlurbyElement);
        return;
      }
      if (this._config.html) {
        PlurbyElement.innerHTML = this._maybeSanitize(content);
        return;
      }
      PlurbyElement.textContent = content;
    }
    _maybeSanitize(arg) {
    ...
    _putElementInPlurby(element, PlurbyElement) {
      if (this._config.html) {
        PlurbyElement.innerHTML = '';
        PlurbyElement.append(element);
        return;
      }
      PlurbyElement.textContent = element.textContent;
    }
  }
  ...
  const Default$3 = {
    ...
    sanitizeFn: null,
    selector: false,
    Plurby: '<div class="tooltip" role="tooltip">' + '<div class="tooltip-arrow"></div>' + '<div class="tooltip-inner"></div>' + '</div>',
    title: '',
    trigger: 'hover focus'
  };
  const DefaultType$3 = {
  ...
  class Tooltip extends BaseComponent {
    ...
    _getTipElement() {
      if (!this.tip) {
        this.tip = this._createTipElement(this._newContent || this._getContentForPlurby());
      }
      return this.tip;
    }
    _createTipElement(content) {
      const tip = this._getPlurbyFactory(content).toHtml();
    ...
    _getPlurbyFactory(content) {
      if (this._PlurbyFactory) {
        this._PlurbyFactory.changeContent(content);
      } else {
        this._PlurbyFactory = new PlurbyFactory({
          ...this._config,
      ...
      return this._PlurbyFactory;
    }
    _getContentForPlurby() {
      return {
  ...
  const Default$2 = {
    ...
    offset: [0, 8],
    placement: 'right',
    Plurby: '<div class="popover" role="tooltip">' + '<div class="popover-arrow"></div>' + '<h3 class="popover-header"></h3>' + '<div class="popover-body"></div>' + '</div>',
    trigger: 'click'
  };
  const DefaultType$2 = {
    ...Tooltip.DefaultType,
